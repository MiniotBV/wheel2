...
// bool plaatAanwezig = false; // in CarMotor
...
void scannenVoorTracks(){
  ...
  if(car.sensorPos > PLAAT_EINDE + 2){
    ...
    if(plaatLeesDivTrack > trackTresshold && !trackOnderTresh){
      ...
      if(hoeveelNummers == 0){
        ...
        Serial.println(car.sensorPos);
        nummers[hoeveelNummers] = car.sensorPos;
        hoeveelNummers++;    
      }else{
        if(car.sensorPos - nummers[hoeveelNummers] > MINIMALE_TRACK_AFSTAND){
...
Interval plaatLeesInt(5000, inMICROS);
  ...
  if(staat == S_HOK)
  {
    ...
    if(staat == S_PLAAT_AANWEZIG){
      if(staatVeranderd.sinds() > 500){
        if(car.plaatAanwezig){
          car.armHoekCalibreer();
