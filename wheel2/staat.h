...
Interval draaienInterval(10, inMILLIS);
...
void updatePlateauRpm()
{
  ...
  if(rpmStaat == AUTO) {targetRpm = autoRpm; return; }
  if(rpmStaat == R33)  {targetRpm = rpm33;   return; }
  if(rpmStaat == R45)  {targetRpm = rpm45;   return; }
}

void setPlateauRpm(float rpm) { autoRpm = rpm; updatePlateauRpm(); }
...
Interval staatVeranderd(1000, inMILLIS);
...
// Interval staatInt(10000, inMICROS);
// void staatFunc(){ //   if(staatInt.loop()){ //   } // }
