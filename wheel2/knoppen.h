...
Interval ledBlinkInterval(0, inMILLIS);
Interval knopAlleInterval(0, inMILLIS);
Interval specialeDisplayActie(0, inMILLIS);
...
const char* knopNaam(int k)
{
  if(k == KNOP_PLAY) return "play";
  if(k == KNOP_DOORSPOEL) return "doorspoel";
  if(k == KNOP_TERUGSPOEL) return "terugspoel";
  if(k == KNOP_BT) return "bt"; 
  if(k == KNOP_RPM) return "rpm";
...
Interval knoppenInt(10000, inMICROS);

void knoppenUpdate()
{
  if(!knoppenInt.loop()) return;

  getKnopData();

  for(int knop = 0; knop < 8; knop++)
  {
    if(knopStaat[knop] == LOSGELATEN && knopIn[knop] == INGEDRUKT )
    {//             KORT INGEDRUKT
    ...
    else if(knopStaat[knop] == INGEDRUKT && knopIn[knop] == LOSGELATEN )
    {//               KORT LOSGELATEN
    ...
    else if(knopStaat[knop] == SUPER_LANG_INGEDRUKT && knopIn[knop] == LOSGELATEN )
    {//                    SUPER LANG LOSGELATEN
      knopStaat[knop] = LOSGELATEN;
      knopAlleInterval.reset();  //led blink
    ...
    else if(knopIn[knop] == LOSGELATEN)
    { //nog ff checken of de knop is losgelaten zodat en lange druk geen loop maakt
      knopStaat[knop] = LOSGELATEN;
    }
  ...
  else
  {
    if( !isOngeveer(potVolumeDiv, 0,   1.0 / 100) )
    {
      ...
      // volume = limitF(volume + (potVolumeDiv * stappenPerOmwenteling), 0, 63);
      amp.volume += round(potVolumeDiv * 100);
      amp.volume = limitInt(amp.volume, 0, 63);        
    }
